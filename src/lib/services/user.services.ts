...
export const hacerMiembro = async (
  usuarioToken: string,
  isMember: boolean,
  adminToken: string
) => {
  try {
    await axios({
      method: 'POST',
      url: `https://ecuaciclismoapp.pythonanywhere.com/api/usuario/setear_miembro/`,
      data: { token_usuario: usuarioToken, miembro: isMember },
      headers: { Authorization: 'Token ' + adminToken },
    })
  } catch (e) {
    console.error(e)
