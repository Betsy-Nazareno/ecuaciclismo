...
export const newPlaceRequest = async (token: string, token_lugar?: string, path_Pdf?: string) => {
    try {
        const response = await axios({
            method: 'POST',
            url: `${process.env.BASE_URL}/api/solicitud/new_solicitud/`,
...
export const getSolicitudes = async (token: string) => {
    try {
        const response = await axios({
            method: 'GET',
            url: `${process.env.BASE_URL}/api/solicitud/get_solicitudes/`,
            headers: { Authorization: 'Token ' + token },
        })
...
export const getSolicitudById = async (token: string, token_solicitud: string) => {
    try {
        const response = await axios({
            method: 'GET',
            url: `${process.env.BASE_URL}/api/solicitud/get_solicitud_by_id/`,
            headers: { Authorization: 'Token ' + token },
            params: {
...
export const responderSolicitud = async (
    ...
    try {
        await axios({
            method: 'POST',
            url: `${process.env.BASE_URL}/api/solicitud/responder_solicitud/`,
...
export const newVerifiedRequest = async (token: string, description: string, image: string, users: any[]) => {
    try {
        const response = await axios({
            method: 'POST',
            url: `${process.env.BASE_URL}/api/solicitud/new_solicitud_verificado/`,
            headers: { Authorization: 'Token ' + token },
            data: {
