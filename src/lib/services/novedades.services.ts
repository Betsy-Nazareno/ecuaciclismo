...
export const agregarNovedad = async (
  ...
  try {
    await axios({
      method: 'POST',
      url: `${process.env.BASE_URL}/api/consejodia/new_novedad/`,
...
export const obtenerNovedades = async (token: string) => {
  try {
    const response = await axios({
      method: 'GET',
      url: `${process.env.BASE_URL}/api/consejodia/get_novedades/`,
...
export const eliminarNovedad = async (token: string, tokenNovedad: string) => {
  try {
    await axios({
      method: 'DELETE',
      url: `${process.env.BASE_URL}/api/consejodia/delete_novedad/`,
