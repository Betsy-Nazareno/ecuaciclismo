...
export const getCiclistasToken = async (token: string) => {
  try {
    const response = await axios({
      method: 'GET',
      url: 'https://fa94-157-100-104-212.ngrok-free.app/api/usuario/token_notificacion_users/',
...
export const getAdminTokens = async (token: string) => {
  try {
    const response = await axios({
      method: 'GET',
      url: 'https://fa94-157-100-104-212.ngrok-free.app/api/usuario/token_notificacion_admins/',
      headers: { Authorization: 'Token ' + token },
    })
    const arrayTokens = response.data?.data || []
    const tokensFiltered = arrayTokens?.filter(
