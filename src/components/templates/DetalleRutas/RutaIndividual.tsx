  ...
  const NegocioCard = ({ negocio }: { negocio: any }) => {
    return (
      <View style={tw`bg-white shadow-lg rounded-lg p-4 mb-4`}>
        <Image
          ...
          resizeMode="cover"
        />
        <Text style={tw`font-semibold text-lg`}>{negocio.nombre}</Text>
        <Text style={tw`text-sm text-gray-500 mt-2`}>{negocio.descripcion}</Text>
      </View>
    )
  }
  ...
  const RutaIndividual = ({ token }: RutaIndividualProps) => {
    ...
    React.useEffect(() => {
      ;(async () => {
        if (authToken && token) {
          ...
          const negociosData = await getNegociosConReservasPorRuta(authToken, token)
    ...
    return !ruta ? (
      ...
      <View style={tw`px-2 mb-4`}>
        ...
        <RutaMapView ubicacion={ruta.ubicacion} negocios={negocios} />
        ...
        {negocios.length > 0 && (
          ...
          <View style={tw`mt-6`}>        
            ...
            <Text style={tw`font-bold text-xl mb-4`}>Negocios</Text>
